...
import { Send, Sparkles, Zap, Copy, Check, Terminal, Layers, LogOut, User } from 'lucide-react';
...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    if (!isLoggedIn) {
      alert('Please login to generate campaigns');
      router.push('/login');
      return;
    }
    ...
    try {
      // Get JWT token (like adding token to headers in MERN)
                ...
                {isLoggedIn ? (
                  <>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-colors text-xs"
                    >
                  ...
                  <>
                    <Link href="/login" className="px-4 py-2 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-colors text-xs">
                      Login
                    </Link>
                    <Link href="/register" className="px-4 py-2 rounded-full bg-indigo-600 hover:bg-indigo-700 transition-colors text-xs font-medium">
                      Sign Up
                    </Link>
                  </>
                )}
